...
export const addSubCategory = async (subCategoryData) => {
  const subCategoryRepository = getRepositoryFactor().get(REPOSITORY_TYPES.SUBCATEGORY);
...
export const getSubCategories = async (filter, options) => {
  ...
  if (filter.search) {
    return subCategoryRepository.getPaginatedListWithSearch(filter, queryOptions);
  } else {
