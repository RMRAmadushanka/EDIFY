...
const CategoryManagement = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <Button onClick={() => showSubcategoryModal()}>Add Subcategory</Button>
            ...
            <Form>
              ...
              <AntForm.Item label="Subcategories">
                {tempSubcategories.length < 10 && !addingSubcategory && (
                  <Button type="dashed" icon={<PlusOutlined />} onClick={() => setAddingSubcategory(true)}>Add Subcategory</Button>
                )}
                <div className="mt-2">
                  {tempSubcategories.map(sub => (
                    <div key={sub.key} className="flex inline-flex justify-between ml-2 items-center mb-2 p-2 border rounded" >
              ...
              {addingSubcategory && (
                <div className="mt-4">
                  <h2 className="text-lg font-semibold mb-2">New Subcategory</h2>
                  <Formik
                    initialValues={{ name: '', featured: false, status: 'Active' }}
                    ...
                    {({ values, setFieldValue, errors, touched, resetForm }) => (
                      <Form>
                        ...
                        <div className="flex justify-end">
                          <Button type="default" onClick={() => setAddingSubcategory(false)} className="mr-2">Cancel</Button>
                          <Button type="primary" htmlType="submit">Add Subcategory</Button>
                        </div>
                      </Form>
                    )}
                  </Formik>
