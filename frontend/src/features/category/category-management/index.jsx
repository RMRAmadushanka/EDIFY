...
const CategoryManagement = () => {
  ...
  const showSubcategoryModal = (subcategory = null) => {
    if (subcategory) {
      setTempSubcategories(tempSubcategories.map(sub => sub.key === subcategory.key ? subcategory : sub));
    }
  ...
  const handleSubcategoryOk = (values, { resetForm }) => {
    if (currentSubcategory) {
      setTempSubcategories(tempSubcategories.map(sub => sub.key === currentSubcategory.key ? values : sub));
    } else {
            ...
            <Form>
              ...
              <AntForm.Item label="Subcategories">
                ...
                {tempSubcategories.map(sub => (
                  <Card key={sub.key} className="mt-2 p-2">
                    <div className="flex justify-between items-center">
                      <div>{sub.name}</div>
                      <Space size="small">
                        <Button size="small" onClick={() => showSubcategoryModal(sub)}>Edit</Button>
                        <Button size="small" danger onClick={() => deleteSubcategory(sub.key)}>Delete</Button>
                      </Space>
                    </div>
                  </Card>
                ))}
